...
function generateQuotePDF(project: Project, calc: ReturnType<typeof calculateProject>) {
  ...
  doc.text("Client Information", margin + 2, y + 6);
  y += 12;

  doc.setFontSize(12);
  doc.text(`Client: ${project.client_name}`, margin, y);
  y += 8;
  doc.text(`Address: ${project.client_address}`, margin, y);
  y += 8;
  doc.text(`Trade: ${TRADE_LABELS[project.trade]}`, margin, y);
  y += 14;
  ...
  doc.rect(margin, y, 170, 8, "F");
  doc.setFontSize(14);
  doc.text("Quote Details", margin + 2, y + 6);
  y += 12;

  doc.setFontSize(12);
  doc.text(`Hourly Rate: $${project.hourly_rate}/hr`, margin, y);
  y += 8;
  doc.text(`Estimated Hours: ${project.estimated_hours}h`, margin, y);
  y += 10;
  ...
  doc.setFontSize(16);
  doc.setTextColor(37, 99, 235);
  doc.text(`Quote Total: $${calc.quote_amount.toLocaleString()}`, margin, y);
  y += 16;
  ...
  doc.save(`quote-${project.client_name.replace(/\s+/g, "-")}.pdf`);
}
